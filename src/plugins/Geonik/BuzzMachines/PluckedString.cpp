...
static double const NoteFreqs[12] = 
  {130.8127, 138.5913, 146.8324, 155.5635, 164.8138, 174.6141, 
   184.9972, 195.9977, 207.6523, 220, 233.0819, 246.9417 };
static double const OctaveMul[10] = 
  { 1.0 / 16, 1.0 / 8, 1.0 / 4, 1.0 / 2, 1.0, 
...
char const *geonik_pluckedstring::describe_value(int const ParamNum, 
						 int const Value) {
  ...
  case mpvDamper:
    sprintf(TxtBuffer, "%.5f", 
	    1.0 - ((double)Value / 256.0) * ((double)Value / 256.0)); 
    break;
  default:
    return NULL; 
  }
...
bool geonik_pluckedstring::process_stereo(float **pin, float **pout, 
					  int numsamples, int mode) {
...
void CTrack::Tick(int ThisTrack) {
  ...
  if(tp.Note == zzub::note_value_off) {
    ...
    if(tp.Slide == zzub::switch_value_on) 
      LastTrack->NoteOn(tp.Note, true);
    else {
      double m = MaxAmp ^ 20; 
      int t;
      for (int c = 0; c < __max(pMachine->numTracks, pMachine->Attr.MaxDyn); c++) {
	if (c < pMachine->numTracks && c != ThisTrack) 
	  continue;
...
void CTrack::Work(float *Dest, int numsamples) {
  ...
  while(numsamples > 0) {
    ...
    if(dp == psWave.pBuffer + psWave.iLength) 
      dp = psWave.pBuffer; 
  }
...
struct geonik_pluckedstring_plugin_info : zzub::info {
  geonik_pluckedstring_plugin_info() {
    ...
    this->author = "George Nicolaidis aka Geonik";
    this->uri = "neil/generator/pluckedstring;1";    
