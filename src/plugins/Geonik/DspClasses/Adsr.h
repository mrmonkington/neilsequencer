...
public:
  ...
  void Init() {
    fMsToSample = (double)dspcSampleRate / 1000.0;
  ...
  void NoteOn(double const a = 1.0) {
    ...
    iCount = iAttack;
    fLinStep = DspCalcLinStep(fAmplitude * dspcAdsr_StartVol,
			      fAmplitude, fInvAttack);
    fLogStep = 1.0;
    iState = sAttack; 
  }
