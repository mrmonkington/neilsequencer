...
void sprayman::InitTrack(int const i)
{
}

void sprayman::ResetTrack(int const i)
{
  InitTrack(i);
}
...
static void DoWork(float *pin, float *pout, sprayman *psprayman, int c)
{
  float *pData=psprayman->Buffer;
  for (int g=0; g<psprayman->Richness; g++)
    {
      CGranule *pGran=psprayman->Granules+g;
      int c1=0;
      while(c1<c)
	{
	  int DelayMax=int(4+pow(DELAY_MAX,0.3+0.7*(64-psprayman->Density)/64.0));
	  int depAttack=1024*(260-psprayman->Attack)/240;
	  int depSustain=1024*(260-psprayman->Sustain)/240;
	  int depRelease=1024*(260-psprayman->Release)/240;
	  ...
	  if (pGran->EnvPoint>=256*GRANULE_SIZE/* && (rand()&7)==0*/)
	      ...
	      if (pGran->Phase>=3)
		{
		  ...
		  pGran->Offset=80+rand()%int(256+pow(OFFSET_MAX,0.5+0.5*psprayman->Spaceyness/64.0));
		  pGran->Delay=rand()%DelayMax;
		  pGran->Phase=0;
		  pGran->DetuneFactor=float(((rand()&1)?-1:+1)*(rand()%(1+10*psprayman->Fatness))*0.00003);
		  if (pGran->DetuneFactor<0) pGran->Offset-=(int)(15000*pGran->DetuneFactor);
		  if (psprayman->Octaviation<50)
		    {
		      if ((rand()&100)<psprayman->Octaviation)
			pGran->DetuneFactor+=0.5;
		    }
		  else
		    {
		      int nRand=rand()&100;
		      if (nRand<psprayman->Octaviation/3)
			pGran->DetuneFactor+=0.5;
		      else
			if (nRand<psprayman->Octaviation*2/3)
	  ...
	  if (pGran->Phase==0) dep=depAttack, pEnv=psprayman->Rise; // attack
	  if (pGran->Phase==1) dep=depSustain; // sustain
	  if (pGran->Phase==2) dep=depRelease, pEnv=psprayman->Fall; // release
	  ...
	  if (c1<c2)
	    {
	      ...
	      int EnvLength=GRANULE_SIZE*256/(depAttack+depSustain+depRelease)+DELAY_MAX;
	      float Amp=float((psprayman->Feedback?1.0:psprayman->WetOut)*EnvLength/((EnvLength+DelayMax/2)*sqrt(psprayman->Richness)));
		  ...
		  for (int i=c1; i<c2; i++)
		    {
		      ...
		      int nPos=(psprayman->Pos-pGran->Offset+i-intDet);
...
void sprayman::WorkTrack(float *pin, float *pout, int numsamples, int const mode)
{
...
bool sprayman::WorkMonoToStereo(float *pin, float *pout, int numsamples, int const mode)
{
...
void sprayman::command(int const i)
{
  _host->message("FSM ScrapMan version 0.26a !\nWritten by Krzysztof Foltman (kf@cw.pl).\n"
...
bool sprayman::process_stereo(float **pin, float **pout, int numsamples, int mode) {
  float tmp_in[1024], tmp_out[2048]; // hardcoded buffer size
  ...
  // WorkMonoToStereo takes input from arg1 and outputs interleaved to arg2.
  bool retval = WorkMonoToStereo(tmp_in, tmp_out, numsamples, mode);
  for (int i = 0; i < numsamples; i++) {
...
struct sprayman_plugin_info : zzub::info {
  sprayman_plugin_info() {
    ...
    this->author = "FSM";
    this->uri = "jmmcd/Effects/SprayMan";
