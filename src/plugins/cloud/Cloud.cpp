...
void Cloud::process_events() {
  ...
  if (gval.offset_devi != paramOffsetDevi->value_none) {
    this->the_cloud->set_offset_devi(pow((float)gval.offset_devi / (float)0xFFFE, 4));
  }
  ...
  if (gval.amp_devi != paramAmpDevi->value_none) {
    this->the_cloud->set_amp_devi(pow((float)gval.amp_devi / (float)0xFFFE, 4));
  }
  ...
  if (gval.length_devi != paramLengthDevi->value_none) {
    this->the_cloud->set_length_devi(pow((float)gval.length_devi / (float)0xFFFE, 4));
  }
  ...
  if (gval.sustain_devi != paramSustainDevi->value_none) {
    this->the_cloud->set_sustain_devi(pow((float)gval.sustain_devi / (float)0xFFFE, 4));
  }
  ...
  if (gval.skew_devi != paramSkewDevi->value_none) {
    this->the_cloud->set_skew_devi(pow((float)gval.skew_devi / (float)0xFFFE, 4));
  }
  ...
  if (gval.rate_devi != paramRateDevi->value_none) {
    this->the_cloud->set_rate_devi(pow((float)gval.rate_devi / (float)0xFFFE, 4));
  }
  ...
  if (gval.pan_devi != paramPanDevi->value_none) {
    this->the_cloud->set_pan_devi(pow((float)gval.pan_devi / (float)0xFFFE, 4));
  }
  if (gval.density != paramDensity->value_none) {
    this->the_cloud->set_density(pow((float)gval.density / (float)0xFFFE, 4));
  }
...
const char *Cloud::describe_value(int param, int value) {
  ...
  case 1:
    ...
    // Offset deviation
  case 2:
    sprintf(str, "%.4f", pow((float)value / (float)0xFFFE, 4));
    break;
    // Amp mean
  case 3:
