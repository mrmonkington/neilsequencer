...
void Cloud::process_events() {
  ...
  if (gval.offset_devi != paramOffsetDevi->value_none) {
    this->the_cloud->set_offset_devi(pow((float)gval.offset_devi / (float)0xFFFE, 2));
  }
  if (gval.amp_mean != paramAmpMean->value_none) {
    this->the_cloud->set_amp_mean((float)gval.amp_mean / (float)0xFFFE);
  }
  if (gval.amp_devi != paramAmpDevi->value_none) {
    this->the_cloud->set_amp_devi(pow((float)gval.amp_devi / (float)0xFFFE, 2));
  }
  if (gval.length_mean != paramLengthMean->value_none) {
    this->the_cloud->set_length_mean(10.0 + (float)gval.length_mean / (float)0xFFFE * 990.0);
  }
  if (gval.length_devi != paramLengthDevi->value_none) {
    this->the_cloud->set_length_devi(pow((float)gval.length_devi / (float)0xFFFE, 2));
  }
  if (gval.sustain_mean != paramSustainMean->value_none) {
    this->the_cloud->set_sustain_mean((float)gval.sustain_mean / (float)0xFFFE);
  }
  if (gval.sustain_devi != paramSustainDevi->value_none) {
    this->the_cloud->set_sustain_devi(pow((float)gval.sustain_devi / (float)0xFFFE, 2));
  }
  if (gval.skew_mean != paramSkewMean->value_none) {
    this->the_cloud->set_skew_mean((float)gval.skew_mean / (float)0xFFFE);
  }
  if (gval.skew_devi != paramSkewDevi->value_none) {
    this->the_cloud->set_skew_devi(pow((float)gval.skew_devi / (float)0xFFFE, 2));
  }
  if (gval.rate_mean != paramRateMean->value_none) {
    this->the_cloud->set_rate_mean((float)gval.rate_mean / (float)0xFFFE * 8.0 - 4.0);
  }
  if (gval.rate_devi != paramRateDevi->value_none) {
    this->the_cloud->set_rate_devi(pow((float)gval.rate_devi / (float)0xFFFE, 2));
  }
  if (gval.pan_mean != paramPanMean->value_none) {
    this->the_cloud->set_pan_mean((float)gval.pan_mean / (float)0xFFFE * 2.0 - 1.0);
  }
  if (gval.pan_devi != paramPanDevi->value_none) {
    this->the_cloud->set_pan_devi(pow((float)gval.pan_devi / (float)0xFFFE, 2));
  }
  if (gval.density != paramDensity->value_none) {
    this->the_cloud->set_density(pow((float)gval.density / (float)0xFFFE, 2));
  }
...
const char *Cloud::describe_value(int param, int value) {
  ...
  case 0:
    ...
    // Offset mean
  case 1:
    sprintf(str, "%.2f%%", ((float)value / (float)0xFFFE) * 100.0);
    break;
    // Offset deviation
  case 2:
    sprintf(str, "%.4f", pow((float)value / (float)0xFFFE, 2));
    break;
    // Amp mean
  case 3:
    sprintf(str, "%.2f", ((float)value / (float)0xFFFE));
    break;
    // Amp deviation
  case 4:
    sprintf(str, "%.4f", pow((float)value / (float)0xFFFE, 2));
    ...
    // Length mean
  case 5:
    sprintf(str, "%.2fms", (10.0 + (float)value / (float)0xFFFE * 990.0));
    break;
    // Length deviation
  case 6:
    sprintf(str, "%.4f", pow((float)value / (float)0xFFFE, 2));
  ...
  case 10:
    ...
    // Rate mean
  case 11:
    sprintf(str, "%.2f", ((float)value / (float)0xFFFE) * 8.0 - 4.0);
    break;
    // Rate deviation
  case 12:
    sprintf(str, "%.4f", pow((float)value / (float)0xFFFE, 2));
    ...
    // Pan mean
  case 13:
    sprintf(str, "%.2f", ((float)value / (float)0xFFFE) * 2.0 - 1.0);
    break;
    // Pan deviation
  case 14:
    sprintf(str, "%.4f", pow((float)value / (float)0xFFFE, 2));
