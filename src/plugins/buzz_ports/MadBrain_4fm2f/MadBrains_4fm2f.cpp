...
void oscillator::tick(int algo, int oscnum, int sr)
{
	...
	freq = (int) ((((float)fc.freq + (float)(fc.fine)/250.0f) * (float) fc.note) * 268435456.0f);
...
C4fm2f::C4fm2f() {
  C4fm2f_cmachine = new mi;
  pz = this;


  global_values = &C4fm2f_cmachine->gval;
  track_values = C4fm2f_cmachine->tval;
}
C4fm2f::~C4fm2f() {
}
void C4fm2f::process_events() {
  C4fm2f_cmachine->Tick();
}
void C4fm2f::init(zzub::archive *arc) {
  C4fm2f_cmachine->Init(0);
}
bool C4fm2f::process_stereo(float **pin, float **pout, int numsamples, int mode) {
  ...
  // to the second.
  bool retval = C4fm2f_cmachine->Work(pout[0], numsamples, mode);
...
const char * C4fm2f::describe_value(int param, int value) {
  return C4fm2f_cmachine->DescribeValue(param, value);
}


void C4fm2f::set_track_count(int n) {
  C4fm2f_cmachine->SetNumTracks(n);
}
void C4fm2f::stop() {
  C4fm2f_cmachine->Stop();
}

void C4fm2f::destroy() { 
  delete C4fm2f_cmachine;
  delete this; 
}

void C4fm2f::attributes_changed() {
  C4fm2f_cmachine->AttributesChanged();
}
