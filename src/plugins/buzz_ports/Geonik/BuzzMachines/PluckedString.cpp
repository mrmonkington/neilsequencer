...
public:
  ...
  Attributes avals;
private:
  CMachine *pluckedstring_cmachine;
};
...
// 	"Geonik's Plucked String","Pluck String","George Nicolaidis aka Geonik","About..." };
...
void CMachine::Init() {
  dspcSampleRate	= pz->_master_info->samples_per_second;
	...
	double b	= 2.0 - cos(10 * 2 * PI / (double)pz->_master_info->samples_per_tick);
...
bool CMachine::Work(float *Dest, int NumSamples, int const Mode) {
...
void CTrack::NoteOn(byte Note, bool Slide) {
	...
	psWave.SetDelay((ulong)floor((double)pz->_master_info->samples_per_second / freq));
...
void CTrack::Work(float *Dest, int numsamples) {
	...
	while(numsamples > 0) {
		...
		do {
			...
			*dp++ = (float)(d * (v + lv));
			lv = v;
			*Dest++ = (float)(downscale * v * a); } while(--c);
...
geonik_pluckedstring::geonik_pluckedstring() {
  pluckedstring_cmachine = new CMachine;
  pluckedstring_cmachine->pz = this;


  global_values = &pluckedstring_cmachine->Param;
  track_values = pluckedstring_cmachine->TrackParam;
  attributes = (int *)&pluckedstring_cmachine->Attr;
}
geonik_pluckedstring::~geonik_pluckedstring() {
}
void geonik_pluckedstring::process_events() {
  pluckedstring_cmachine->Tick();
}
void geonik_pluckedstring::init(zzub::archive *arc) {
  pluckedstring_cmachine->Init();
}
bool geonik_pluckedstring::process_stereo(float **pin, float **pout, int numsamples, int mode) {
  ...
  // second.
  bool retval = pluckedstring_cmachine->Work(pout[0], numsamples, mode);
...
const char * geonik_pluckedstring::describe_value(int param, int value) {
  return pluckedstring_cmachine->DescribeValue(param, value);
}


void geonik_pluckedstring::set_track_count(int n) {
  pluckedstring_cmachine->SetNumTracks(n);
}
void geonik_pluckedstring::stop() {
  pluckedstring_cmachine->Stop();
}

void geonik_pluckedstring::destroy() { 
  delete pluckedstring_cmachine;
  delete this; 
}

void geonik_pluckedstring::attributes_changed() {
  pluckedstring_cmachine->AttributesChanged();
}
...
struct geonik_pluckedstring_plugin_info : zzub::info {
  ...
  virtual zzub::plugin* create_plugin() const { return new geonik_pluckedstring(); }
