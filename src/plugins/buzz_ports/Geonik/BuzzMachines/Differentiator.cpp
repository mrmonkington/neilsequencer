/*
 *		Differentiator plug-in for Buzz
 *
...
CMachineParameter const mpAmp = {
	pt_byte,"Amp","Amp (0=-64dB, 40=0, 80=64dB, Default = 0dB)",
	0x0,0x80,0xFF,MPF_STATE,0x40 };

CMachineParameter const *mpArray[] = {
	&mpAmp };

enum mpValues { mpvAmp = 0 };
...
CMachineInfo const MachineInfo = { 
	...
	0,0,1,0,mpArray,0,maArray,
	"Geonik's Differentiator","Differenciator","George Nicolaidis aka Geonik",NULL };
...
public:
	...
	virtual void		 Tick();
	virtual bool		 Work(float *pin,int numsamples,int const mode);
	virtual char const	*DescribeValue(int const param, int const value);

private:
	double			difAmp;
	double			difLastSample;
...
char const *Effect::DescribeValue(int const ParamNum, int const Value) {
	...
	switch(ParamNum) {
	case mpvAmp:
		sprintf(TxtBuffer,"%d dB", Value-64);
		break;
	default:
		return NULL; }
	return TxtBuffer; }
...
void Effect::Tick() {
	...
	if(Param.Amp != mpAmp.NoValue) { 
		difAmp = pow(2.0,(double)(Param.Amp-64)/6.0); } }


#pragma optimize ("a", on)

bool Effect::Work(float *pb,int numSamples,int const Mode) {

	if(!(Mode & WM_READ)) {
		return false; }

	double const a = difAmp;
	double		 ls= difLastSample;

	do {
		double s = *pb;
		*pb++ = (float)((s-ls)*a); ls=s;
	} while(--numSamples);
