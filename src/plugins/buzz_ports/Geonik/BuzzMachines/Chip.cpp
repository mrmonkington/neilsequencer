...
CMachineParameter const mpC0		= { pt_byte,"Control1","Control 1 (0 to 80)",0,0x80,0xFF,MPF_STATE,0x40 };
CMachineParameter const mpC1		= { pt_byte,"Control2","Control 2 (0 to 80)",0,0x80,0xFF,MPF_STATE,0x40 };
CMachineParameter const mpC2		= { pt_byte,"Control3","Control 3 (0 to 80)",0,0x80,0xFF,MPF_STATE,0x40 };

CMachineParameter const *mpArray[]	= { &mpNote,&mpC0,&mpC1,&mpC2 };
enum					 mpValues	  { mpvNote,mpvC0 };

#pragma pack(1)		
struct CParameters					  { byte Note, C[c_numControls]; };
...
CMachineAttribute const  maMaxDyn	= { "Dynamic Channels",0,c_maxDynTracks,8 };
...
#pragma pack(1)		
struct CAttributes					  { int MaxDyn,DefVol,DynRange; };
#pragma pack()
...
CMachineInfo const miMachineInfo = { 
	...
	1,c_maxTracks,0,1+c_numControls,mpArray,3,maArray,
	"Geonik's Chip","Chip","George Nicolaidis aka Geonik","About..." };
...
void CMachine::Init(CMachineDataInput * const pi) {
	...
	iDynTracks		= 0;								// Waves
	for (int c = 0; c < c_maxTracks; c++)	{			// Tracks
...
char const *CMachine::DescribeValue(int const pc, int const iv) {
	...
	case mpvC0:
		...
		if(v) sprintf(b,"%.1f : %.1f", (float)(50+v), (float)(50-v));
		else  return "Square";
		break;
	case mpvC0+1:
		v = ControlByteToDouble(1,iv);
		if(v < 1000) sprintf(b,"%.1f ms", (float)v);
		else		 sprintf(b,"%.1f sec", (float)(v*(1.0/1000)));
		break;
	default:
		return NULL; }
	return b; }
...
void CMachine::AttributesChanged() {
	if(cAttributes.MaxDyn < iDynTracks) {
		for(int i=cAttributes.MaxDyn; i<iDynTracks; i++) aTracks[i].Free(); }
	iDynTracks = __max(iRealTracks,cAttributes.MaxDyn);
	fSilentEnough = c_maxAvailability * pow(2.0,(double)cAttributes.DynRange/6.0); }
...
void CMachine::Stop() { 
	for (int c=0; c<iRealTracks; c++) aTracks[c].Stop(); }
...
void CMachine::MidiNote(int const channel, int const value, int const velocity) {
	...
	oct -= 1; note += 1;
	aTracks[0].pPlayingTrack->NoteOn(((oct << 4) + note),aTracks,aTracks[0].pParameters); }
...
void CTrack::WorkTick() {
	if(bPlaying) {
		...
		if(fAvailability < pMachine->fSilentEnough) cEnv.Stop();
...
void CTrack::Tick(int const ThisTrack) {
	for(int i=0; i<c_numControls; i++) {
		if(pParameters->C[i] != 0xff) {				// Waring: Hardcoded novalue
			bControl[i] = pParameters->C[i];
			fControl[i] = ControlByteToDouble(i,bControl[i]);
			pPlayingTrack->ControlChange(i,fControl[i],(pParameters->Note != NOTE_NO)); } }
