...
CMachineInfo const MachineInfo = { 
	...
	"Geonik's Expression","Expression","George Nicolaidis aka Geonik",NULL };
...
void Effect::Tick() {
	...
	if(Param.Inertia != mpInertia.NoValue) {
		gInertiaSamples		= (double)pMasterInfo->SamplesPerSec * pow(10.0,(double)Param.Inertia*(1.0/20)) * (1.0 / 5000.0);
...
inline double Effect::DetectLevel(float *inb, int ns) {
	...
	do {
		double v = *inb++;
		q = c1*v*v*a + c2 * q; } while(--ns);

	rmsQ = q;
	return sqrt(q); }


inline void Effect::Filter(float *pb, int const ns_, double const step) {
	double const a  = gDrive;
	double		 L  = fLowpass;
	double		 B  = fBandpass;
	double		 H  = fHighpass;
...
bool Effect::Work(float *pin,int numSamples,int const Mode) {
