...
modulator::modulator() {
  modulator_cmachine = new mi;
  pz = this;


  track_values = modulator_cmachine->tval;
}
modulator::~modulator() {
}
void modulator::process_events() {
  modulator_cmachine->Tick();
}
void modulator::init(zzub::archive *arc) {
  modulator_cmachine->Init(0);
}
bool modulator::process_stereo(float **pin, float **pout, int numsamples, int mode) {
  ...
  // FIXME ugly... WorkMonoToStereo interleaves the output from a mono input.
  float tmp[20000];
  bool retval = modulator_cmachine->WorkMonoToStereo(pin[0], tmp, numsamples, mode);
...
const char * modulator::describe_value(int param, int value) {
  return modulator_cmachine->DescribeValue(param, value);
}


void modulator::set_track_count(int n) {
  modulator_cmachine->SetNumTracks(n);
}
void modulator::stop() {
  modulator_cmachine->Stop();
}

void modulator::destroy() { 
  delete modulator_cmachine;
  delete this; 
}

void modulator::attributes_changed() {
  modulator_cmachine->AttributesChanged();
}
