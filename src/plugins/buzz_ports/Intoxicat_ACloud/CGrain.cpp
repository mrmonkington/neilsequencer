...
void CGrain::SetWave(int wave, int stereo, const zzub::wave_level *pW)//should be void
{
	Wave = wave;
	IsStereo = stereo;
	pwv = pW;
...
//	spsMix = (float)*pSPS/(float)pwv->samples_per_second;
	
//	Pos = (float)pwv->sample_count * Offset;
	Pos = float(Offset);
	...
	if ((Duration+(iPos/Rate)) > pwv->sample_count/Rate){//mono check.
                //Duration -= (Duration*Offset);//
		Duration = pwv->sample_count-(pwv->sample_count-iPos);//Hmmm....
		if (Rate > 1.0) Duration = (int)((pwv->sample_count-iPos)/Rate);
		}
	//if (IsStereo && (Duration+(iPos*0.5f)) > (int)(pwv->sample_count*Rate)*0.5f){
   
	//	Duration = (int)((pwv->sample_count-iPos)*0.5f);
	//	if (Rate > 1.0) Duration = (int)(((pwv->sample_count-iPos)*0.5f)/Rate);
	//	}
...
void CGrain::GetNext()//(const zzub::wave_level *pwv, int SPS)
{
//return 0.0f;//	NULL;
...
//	pwv = pW;
	...
	else //mono - hmm branching - shoudl this be replaced with if (!IsStereo)
	{
		x =	f2i(Pos);
		x2 = x + 1;
		if(x2 >= pwv->sample_count) x2 = 0;

		outL = pwv->samples[x] + ((Pos) - (float)x)*(pwv->samples[x2] - pwv->samples[x]);
		outR = outL;
	}
...
void CGrain::GenerateAdd(float * psamples, int numsamples, const zzub::wave_level *pW)
{
	pwv = pW;
...
void CGrain::Generate(float * psamples, int numsamples, const zzub::wave_level *pW)
{
	pwv = pW;
	if (!pwv || lastNumSamples != pwv->sample_count){
		IsActive = false;
