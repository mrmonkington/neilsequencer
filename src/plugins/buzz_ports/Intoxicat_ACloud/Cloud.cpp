...
//___________________________________________________________________
//
inline int mi::FindGrain(int maxg)
{
...
inline int mi::SetNextGrain(int dens)
{
        float r = ((float)rand() * 2.0f / RAND_MAX);//range 0/2.0
	int t = (int)((pz->_master_info->samples_per_second / dens) * r);
	return t + 1;
}

inline int mi::CountGrains()
{
	int c = 0;
...
inline int mi::SetDens(int mode)
{
	if (mode == 0) return density;

	return (int)((220500.0f/gdur) * densfactor) + 1;
}
inline bool mi::CheckActivGrains(int maxg)
{
	for(int i=0; i < maxg; i++) if (Grain[i].IsActive == 1) return true;
	return false;
}

double mi::SetOffset(int wave, int wnum) //start end
{
	float s = 0.0f;
	float e = 0.0f;
	double rtn;
	const zzub::wave_level *pwl = pz->_host->get_wave_level(wnum,0);
	...
	offsCount += (offsInc * skiprate * ((float)pwl->samples_per_second / (float)pz->_master_info->samples_per_second));
...
inline int mi::SelectWave(int mix)
{
...
void mi::SelectWave2(int mix, WAVESEL * Wv)
{
...
inline float mi::GetRandRate()//this should be rewritten
{
	//float rtn = 0.0f;
...
float mi::GetRandPan()//thisis a bit crap i think... 
{
	float pwidth;
	float r = (float)rand()/RAND_MAX;
	//pwidth *= r;
...
void mi::Init(CMachineDataInput * const pi)
{
	...
	// pz->_host->set_track_count(ThisMachine,2);
	...
	for(int i=0; i<127;i++)
	{
		Grain[i].SetMiPointers(&pz->_master_info->samples_per_second);
		Grain[i].Init();
	}
...
void mi::Save(CMachineDataOutput * const po){NULL;};
...
void mi::Tick()
{
  ...
  int o = pz->_master_info->samples_per_tick;

  if (gval.note != NOTE_NO) //this is going to change to note
  {
	  if(gval.note != 255)
	  {
		 //printf ("\nNOTE!: : %d",gval.note);
  ...
  if (gval.gduration != 0xFFFF) gdur = gval.gduration * (pz->_master_info->samples_per_second/44100);
...
//	WAVESEL *ws;//struct test
	...
	if (gcount > gnext && cloud_is_playing)//check if grain should be generated thsi work
	{
		do
		{
			...
			if (g >= 0)
			{
				...
				if (pz->_host->get_wave_level(w,0))
				{	
					...
					Grain[g].SetWave(w, (pz->_host->get_wave(w)->flags & WF_STEREO), pz->_host->get_wave_level(w,0));
					Grain[g].SetEnv(Grain[g].Duration, envamt, envq);
					Grain[g].SetAmp(ampt, ampb, pz->_host->get_wave(w)->volume);
					Grain[g].IsActive = 1;
				}
			}
	...
	{
		memset(psamples, 0, (numsamples*2) * sizeof(float)); //clear the last buffer
		return false;
	}
	
	memset(psamples, 0, (numsamples*2) * sizeof(float));//????
	
	for(int j=0;j<maxgrains;j++) //can the isfirstinwork check be made more efficient?
	{
		//Grain[j].GenerateAdd(psamples, numsamples);
		if(Grain[j].IsActive == 1 && IsFirstInWork) Grain[j].Generate(psamples, numsamples, pz->_host->get_wave_level(Grain[j].Wave,0));
		if(Grain[j].IsActive == 1 && !IsFirstInWork) Grain[j].GenerateAdd(psamples, numsamples, pz->_host->get_wave_level(Grain[j].Wave,0));
...
char const *mi::DescribeValue(int const param, int const value)
{
	...
	case 2:
		sprintf(txt, "%d %s",value, pz->_host->get_wave_name(value));
		return txt;
		break;
	case 3:		
	...
	case 12:
		...
		break;
	case 13:		
		sprintf(txt, "%.1fms", (value/(float)pz->_master_info->samples_per_second)*1000);
		return txt;
		break;
	case 14:		
		sprintf(txt, "%.1fms", (value/(float)pz->_master_info->samples_per_second)*1000);
