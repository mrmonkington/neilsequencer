...
{
	float	*pStart[] = {pStereo[0], pStereo[1]};
	int		iTotal=iCount;
	...
	if( m_fLeftVolumeRamp!=0.0f || m_fRightVolumeRamp!=0.0f )
	{
		...
		while( n-- )
		{
			...
			*pStart[0]++ += (lastl=f*m_fLeftVolume*fAmp);
			*pStart[1]++ += (lastr=f*m_fRightVolume*fAmp);
			m_fLeftVolume+=m_fLeftVolumeRamp;
			m_fRightVolume+=m_fRightVolumeRamp;
		}
	...
	if( iCount>0 )
	{
		...
		while( iCount-- )
		{
			...
			*pStart[0]++ +=(lastl=f*fLeft);
			*pStart[1]++ +=(lastr=f*fRight);
		}
	}
	m_fMaybeLastLeftSample = lastl;
	m_fMaybeLastRightSample = lastr;

	AddFadeOut_Stereo( pStereo, iTotal );
}

void	CAmp::AmpAndAdd_StereoToStereo( float **pStereo, float **pSrc, int iCount, float fAmp )
{
	float	*pStart[] = {pStereo[0], pStereo[1]};
	float	*pI[] = {pSrc[0], pSrc[1]};
	int		iTotal=iCount;
	...
	if( m_fLeftVolumeRamp!=0.0f || m_fRightVolumeRamp!=0.0f )
	{
		...
		while( n-- )
		{
			...
			*pStart[0]++ += (lastl=lf*m_fLeftVolume*fAmp);
			*pStart[1]++ += (lastr=rf*m_fRightVolume*fAmp);
			m_fLeftVolume+=m_fLeftVolumeRamp;
			m_fRightVolume+=m_fRightVolumeRamp;
		}
	...
	if( iCount>0 )
	{
		...
		while( iCount-- )
		{
			...
			*pStart[0]++ +=(lastl=lf*fLeft);
			*pStart[1]++ +=(lastr=rf*fRight);
		}
	}
	m_fMaybeLastLeftSample = lastl;
	m_fMaybeLastRightSample = lastr;
...
void	CAmp::AmpAndMove_ToStereo( float **pStereo, float *pSrc, int iCount, float fAmp )
{
	...
	if( m_fLeftVolumeRamp!=0.0f || m_fRightVolumeRamp!=0.0f )
	{
		...
		while( n-- )
		{
			...
			*pStart[0]++ = f*m_fLeftVolume*fAmp;
			*pStart[1]++ = f*m_fRightVolume*fAmp;
			m_fLeftVolume+=m_fLeftVolumeRamp;
			m_fRightVolume+=m_fRightVolumeRamp;
		}
	...
	m_fMaybeLastRightSample = *--pStart[1];
	m_fMaybeLastLeftSample = *--pStart[0];
...
void	CAmp::AmpAndMove_StereoToStereo( float **pStereo, float **pSrc, int iCount, float fAmp )
{
	...
	if( m_fLeftVolumeRamp!=0.0f || m_fRightVolumeRamp!=0.0f )
	{
		...
		while( n-- )
		{
			...
			*pStart[0]++ = lf*m_fLeftVolume*fAmp;
			*pStart[1]++ = rf*m_fRightVolume*fAmp;
			m_fLeftVolume+=m_fLeftVolumeRamp;
			m_fRightVolume+=m_fRightVolumeRamp;
		}
