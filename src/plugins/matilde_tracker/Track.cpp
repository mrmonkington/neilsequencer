...
int	CTrack::NewNote( bool oRetrig )
{
	...
	if( m_pChannel==NULL )
	{
		...
		m_pChannel->m_PanningEnvelope.ReadEnvelope( m_pMachine->_host, m_iInstrument, 1 );
		m_pChannel->m_PitchEnvelope.ReadEnvelope( m_pMachine->_host, m_iInstrument, 2 );
	}
	...
	if( m_pSample )
	{
		...
		if( !oRetrig )
		{
			m_fFreq      =(float) pow(2.0f,(CONVNOTE(m_iBaseNote)-CONVNOTE(m_pSample->GetRootNote()))/12.0f) * m_pSample->GetRootFrequency() / m_pMachine->_master_info->samples_per_second;
			m_fBaseFreq  =m_fFreq;
...
void	CTrack::Tick( CTrackVals &tv )
{
	...
	if( !NONOTE(m_Vals.note) )
	{
		...
		else if( thesecommands&(1<<3) /*(command==3)||(command==5)*/ )
		{
			if( m_pSample )
			{
				m_fWantedFreq=(float) pow(2.0f,(CONVNOTE(m_Vals.note)-CONVNOTE(m_pSample->GetRootNote()))/12.0f) * m_pSample->GetRootFrequency() / m_pMachine->_master_info->samples_per_second;
			}
			m_iBaseNote=m_Vals.note;
		}
		...
		switch (command)
		{
			...
			case 0x4:
				if( argument!=0 )
				{
					...
					if( (argument&0xF0)!=0 )
						m_fVibratoSpeed=(argument>>4)*(argument>>4)*2*float(M_PI)/256.0f;
				}
				break;
			case 0x6:
				if( argument!=0 )
				{
					...
					if( (argument&0xF0)!=0 )
						m_fPanSpeed=(argument>>4)*(argument>>4)*2*float(M_PI)/256.0f;
				}
				break;
			case 0x7:
				if( argument!=0 )
				{
					...
					if( (argument&0xF0)!=0 )
						m_fTremoloSpeed=(argument>>4)*(argument>>4)*2*float(M_PI)/256.0f;
				}
				break;
			case 0x8:
			...
			case 0x17:
				if( argument )
				{
					if( m_pSample )
					{
						m_fFreq    =(float) pow(2.0f,(CONVNOTE(m_Vals.note)-CONVNOTE(m_pSample->GetRootNote()))/12.0f) * (m_pSample->GetRootFrequency()*argument) / m_pMachine->_master_info->samples_per_second;
						m_fBaseFreq=m_fFreq;
					}
				}
				break;
			...
			case 0x24:
				if( argument!=0 )
				{
					...
					if( (argument&0xF0)!=0 )
						m_fCutoffLFOSpeed=(argument>>4)*(argument>>4)*2*float(M_PI)/256.0f;
				}
				break;
			case 0x25:
			...
			case 0x2C:
				if( argument!=0 )
				{
					...
					if( (argument&0xF0)!=0 )
						m_fResonanceLFOSpeed=(argument>>4)*(argument>>4)*2*float(M_PI)/256.0f;
				}
				break;
			case 0x2D:
...
void	CTrack::ProcessRetrig( int retrig )
{
	...
	{
		m_pChannel->SetRampTime( int(m_pMachine->m_Attributes.iVolumeRamp*m_pMachine->_master_info->samples_per_second/1000.0f) );
	}
	else
	{
	...
	{
		m_pChannel->m_Filter.SetSampleRate( m_pMachine->_master_info->samples_per_second );
...
//				if( m_pChannel->m_pWaveInfo->Flags&WF_STEREO )
				...
				{
					m_fFreq=m_fBaseFreq=float(m_pSample->GetSampleLength())/float(m_pMachine->_master_info->samples_per_tick*m_iLoopStretch);
				...
				m_pChannel->m_VolumeEnvelope.ReadEnvelope( m_pMachine->_host, m_iInstrument, 0 );
	...
	if( m_iLoopStretch!=-1 && m_iLoopStretch && m_oLoopStretchTrack )
	{
		...
		{
			//m_fFreq=m_fBaseFreq=float(m_pChannel->m_pWaveLevel->numSamples)/float(m_pMachine->pMasterInfo->SamplesPerTick*m_iLoopStretch);
			m_fFreq=m_fBaseFreq=float(m_pChannel->m_pSample->GetSampleLength())/float(m_pMachine->_master_info->samples_per_tick*m_iLoopStretch);
			retrig|=RETRIG_FREQ;
		}
	}
			...
			case 0x0:
				if (argument!=0)
				{
					if( m_pChannel->m_pSample && m_pChannel->m_pSample->IsStillValid() )
					{
						...
						{
							case 0:
								m_fFreq=(float) pow(2.0f,(CONVNOTE(m_iBaseNote)-CONVNOTE(m_pChannel->m_pSample->GetRootNote()))/12.0f) * m_pChannel->m_pSample->GetRootFrequency() / m_pMachine->_master_info->samples_per_second;
								break;
							case 1:
								m_fFreq=(float) pow(2.0f,(CONVNOTE(m_iBaseNote)-CONVNOTE(m_pChannel->m_pSample->GetRootNote())+(argument>>4))/12.0f) * m_pChannel->m_pSample->GetRootFrequency() / m_pMachine->_master_info->samples_per_second;
								break;
							case 2:
								m_fFreq=(float) pow(2.0f,(CONVNOTE(m_iBaseNote)-CONVNOTE(m_pChannel->m_pSample->GetRootNote())+(argument&0x0F))/12.0f) * m_pChannel->m_pSample->GetRootFrequency() / m_pMachine->_master_info->samples_per_second;
								break;
						}
						retrig|=RETRIG_FREQ;
					}
