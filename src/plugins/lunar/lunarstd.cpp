...
// if types differ on your platform, cast. don't change the declaration.
void *lunar_memset(void *b, int c, lunar_size_t len) { return memset(b,c,len); }
void *lunar_memcpy(void *b, const void *a, lunar_size_t num) { return memcpy(b,a,num); }
void *lunar_calloc(lunar_size_t nelem, lunar_size_t elsize) { return calloc(nelem, elsize); }
void *lunar_malloc(lunar_size_t len) { return malloc(len); }
void lunar_free (void *p) { return free(p); }
void *lunar_realloc(void *ptr, lunar_size_t len) { return realloc(ptr,len); }
// math
float lunar_pow(float a, float b) { return std::pow(a,b); }
float lunar_log(float x) { return std::log(x); }
float lunar_log2(float x) { return std::log(x) / std::log(2.0); }
...
// other:
int lunar_rand(void) { return std::rand(); }
