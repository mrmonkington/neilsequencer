...
import zzub
from pattern import PatternView
...
class SequencerPanel(gtk.VBox):
	...
	__view__ = dict(
			label = "SequenceMatrix",
			stockid = "aldrin_sequencematrix",
			shortcut = 'F4',
			order = 4,
	)
	...
	def draw(self, ctx):
		...
		while x < h:
			if (i % (16*self.step)) == 0:
				...
				drawable.draw_line(gc, 0, x-1, h, x-1) #
		...
		for track_index in range(self.starttrack, len(tracklist)):
			...
			drawable.draw_rectangle(gc, True, y, 0, SEQTRACKSIZE, SEQLEFTMARGIN-1) #
			gc.set_foreground(pen)
			drawable.draw_rectangle(gc, False, y, 0, SEQTRACKSIZE, SEQLEFTMARGIN-1) #
			gc.set_foreground(textcolor)
			layout.set_text(title)
			px,py = layout.get_pixel_size()
			drawable.draw_layout(gc, y + SEQTRACKSIZE/2 - py/2, SEQLEFTMARGIN-4 -px, layout) #
			...
			for pos, value in track.get_event_list():
				...
				if not bb:
					...
					if value < 0x10:
						...
						bb.draw_rectangle(gc, True, 0, 0, SEQTRACKSIZE-1, psize-1) #
					else:
						...
						bb.draw_rectangle(gc, True, 0,0, SEQTRACKSIZE-2, psize-2) #
						...
						gc.set_foreground(cm.alloc_color('#%02X%02X%02X' % (int(r*255),int(g*255),int(b*255))))
						bb.draw_rectangle(gc, False, 0, 0, SEQTRACKSIZE-2, psize-2) #
						ofs = 0
