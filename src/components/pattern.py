...
class PatternView(gtk.DrawingArea):
	...
	def __init__(self, panel, machine, pattern):
		"""
		Initialization.
		"""
...
#			return True
	...
	def init_values(self):
		...
		datasource = self.machine, self.pattern
		if datasource:
...
#		return
		...
		if not rows:
			rows = self.row_count
		clipy1 = ((row - self.start_row) * self.row_height)
		clipy2 = ((rows - self.start_row) * self.row_height)
