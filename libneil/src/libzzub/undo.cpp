...
namespace zzub {
  ...
  ***/
  ...
  void undo_manager::free_operations(undoableoperation& op) {
    for (size_t i = 0; i < (size_t)op.size(); i++) {
  ...
  void undo_manager::merge_backbuffer_flags(operation_copy_flags flags) {
    ...
    for (size_t i = 0; i < flags.plugin_flags.size(); i++) {
      ...
      if ((size_t)pflags.plugin_id >= front.plugins.size() || front.plugins[pflags.plugin_id] == 0) continue;
	...
	if ((size_t)wflags.level < sw.levels.size()) {
  ...
  void undo_manager::clear_swap_song(zzub::song& song, const operation_copy_flags& flags) {
    ...
    for (size_t i = 0; i < flags.plugin_flags.size(); i++) {
      ...
      if ((size_t)pflags.plugin_id >= song.plugins.size() || song.plugins[pflags.plugin_id] == 0) continue;
	...
	if ((size_t)wflags.level < song.wavetable.waves[wflags.wave]->levels.size())
