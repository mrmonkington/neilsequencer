...
class mdaThruZero : public lunar::fx<mdaThruZero> {
private:
  float param[NPARAMS];
  ...
  void resume() {
    rat = (float)(pow(10.0f, 3.f * param[0] - 2.f) * 2.f / transport->samples_per_second);
    dep = 2000.0f * param[1] * param[1];
    dem = dep - dep * param[4];
    dep -= dem;
...
public:
  ...
  void process_events() {
    if (globals->fParam1)
      param[0] = *globals->fParam1;
    if (globals->fParam2)
      param[1] = *globals->fParam2;
    if (globals->fParam3)
      param[2] = *globals->fParam3;
    if (globals->fParam4)
      param[3] = *globals->fParam4;
    if (globals->fParam5)
      param[4] = *globals->fParam5;
    resume();
  }
