...
void filter::SetFilter_4PoleLP(int CurCutoff, int Resonance) {
  float CutoffFreq = (float)(264 * pow(32.0, CurCutoff / 240.0));
  float cf = (float)CutoffFreq;
  if (cf >= sr / 2) 
    cf = sr / 2;
  if (cf < 33) 
  ...
  B = fB * ncf * ncf;
  float a0 = float(1 / (1 + A + B));
  Biquad.m_b1 = 2 * (Biquad.m_b2 = Biquad.m_b0 = a0);
  Biquad.m_a1 = a0 * (2 - B - B);
  Biquad.m_a2 = a0 * (1 - A + B);
  ...
  B = fB * ncf * ncf;
  a0 = float(1 / (1 + A + B));
  Biquad2.m_b1 = 2 * (Biquad2.m_b2 = Biquad2.m_b0 = 0.35 * a0);
...
void filter::SetFilter_4PoleEQ1(int CurCutoff, int Resonance) {
  ...
  if (cf < 33) 
    cf = (float)(33.0);
  Biquad.SetParametricEQ(cf, (float)(1.0 + Resonance / 12.0),
			 float(6.0 + Resonance / 30.0), sr, 
			 0.4f / (1.0 + (240 - Resonance) / 120.0f));
  Biquad2.SetParametricEQ(float(cf / (1.0 + Resonance / 240.0)),
			  float(1.0 + Resonance / 12.0),
			  float(6.0 + Resonance / 30.0), sr, 0.4f);
}
...
void filter::SetFilter_4PoleEQ2(int CurCutoff, int Resonance) {
  ...
  if (cf < 33) 
    cf=(float)(33.0);
  Biquad.SetParametricEQ(cf, 8.0f, 9.0f, sr, 0.5f);
  Biquad2.SetParametricEQ(float(cf / (3.5 - 2 * Resonance / 240.0)),
			  8.0f, 9.0f, sr, 0.4f);
}
...
void filter::SetFilter_Vocal1(int CurCutoff, int Resonance) {
  ...
  float Cutoff1 = THREESEL(CutoffFreq, 270, 400, 800);
  float Cutoff2 = THREESEL(CutoffFreq, 2140, 800, 1150);
  Biquad.SetParametricEQ(Cutoff1, 2.0f + Resonance / 48.0f,
			 6.0f + Resonance / 24.0f, sr, 0.3f);
  Biquad2.SetParametricEQ(Cutoff2, 2.0f + Resonance / 48.0f,
			  6.0f + Resonance / 24.0f, sr, 0.3f);
}
...
void filter::SetFilter_Vocal2(int CurCutoff, int Resonance) {
  ...
  float Cutoff1 = THREESEL(CutoffFreq, 270, 400, 650);
  float Cutoff2=THREESEL(CutoffFreq, 2140, 1700, 1080);
  Biquad.SetParametricEQ(Cutoff1, 2.0f + Resonance / 56.0f,
			 6.0f + Resonance / 16.0f, sr, 0.3f);
  Biquad2.SetParametricEQ(Cutoff2, 2.0f + Resonance / 56.0f,
			  6.0f + Resonance / 16.0f, sr, 0.3f);
}
