#ifndef SYNTH_TRACK_HPP
#define SYNTH_TRACK_HPP
...
inline float CSynthTrack::GetSample() {
  if (AmpEnvStage) {
    if ((ArpMode>0) && (++Arp_tickcounter>Arp_samplespertick)) 
      ArpTick();
      
    if (syntp->interpolate) {
      ...
      output = ((((((((3 * (d1 - d2)) - d0) + d3) * 0.5f * fractpart) + 
		   ((2 * d2) + d0) - (((5 * d1) + d3) * 0.5f)) * fractpart) + 
		 ((d2 - d0) * 0.5f)) * fractpart + d1) * OSC1Vol;
      ...
      output += ((((((((3 * (d1 - d2)) - d0) + d3) * 0.5f * fractpart) + 
		    ((2 * d2) + d0) - (((5 * d1) + d3) * 0.5f)) * fractpart) + 
		  ((d2 - d0) * 0.5f)) * fractpart + d1) * OSC2Vol;
    }
    else {
    ...
    if (syntp->vcf_type > 9) 
      return m_filter.res2(output) * GetEnvAmp();
