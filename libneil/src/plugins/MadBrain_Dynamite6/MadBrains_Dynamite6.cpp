...
inline float channel::advance()
{
    ...
    if (routing & 1) {
        soap3 = pipes[2].generate(soap) + pipes[3].generate(soap) + pipes[4].generate(soap) + pipes[5].generate(soap);
    } else {
        soap3 = pipes[2].generate(pipes[3].generate(pipes[4].generate(pipes[5].generate(soap))));
    }
