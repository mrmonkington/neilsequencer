...
void CEnvelope::ReadEnvelope(zzub::host *host, int iIns, int iEnvIndex) {
  ...
  for (i = 0; i < m_iEnvelopeSize; i += 1 ) {
    unsigned short x, y;
    int	flags;
    host->get_envelope_point(iIns, iEnvIndex, i, x, y, flags);
    m_pEnvelope[i].m_fPos = x / 65535.0f;
    m_pEnvelope[i].m_fValue = y / 65535.0f;
    m_pEnvelope[i].m_oSustain = (flags & zzub::envelope_flag_sustain) ? true : false;
  }
}
...
float CEnvelope::GetCurrentLevel(int iCount) {
  if (m_iEnvelopeSize < 2)
    return 1.0f;
  while ((m_fEnvelopeTime > m_pEnvelope[m_iEnvelopeIndex + 1].m_fPos) && 
	 (m_iEnvelopeIndex < m_iEnvelopeSize)) {
  ...
  float t = (m_fEnvelopeTime - m_pEnvelope[m_iEnvelopeIndex].m_fPos) / 
    (m_pEnvelope[m_iEnvelopeIndex + 1].m_fPos - 
     m_pEnvelope[m_iEnvelopeIndex].m_fPos);
  m_fEnvelopeTime += iCount * m_fEnvelopeSpeed;
  if (m_fEnvelopeTime > 1.0f)
    m_fEnvelopeTime = 1.0f;
  return (m_pEnvelope[m_iEnvelopeIndex + 1].m_fValue - 
	  m_pEnvelope[m_iEnvelopeIndex].m_fValue) * 
    t + m_pEnvelope[m_iEnvelopeIndex].m_fValue;
}
