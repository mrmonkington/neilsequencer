...
bool CBuzzInstrument::IsSampleStillValid(CBuzzSample *pSmp) {
  const zzub::wave_level *wl = 
    m_pTracker->_host->get_nearest_wave_level(m_iInsNum, pSmp->m_iNote);
  if (wl)
    return m_pWaveInfo == m_pTracker->_host->get_wave(m_iInsNum) && 
      pSmp->m_pWaveLevel == wl && 
      wl->sample_count == pSmp->m_iSavedNumSamples && 
      wl->samples == pSmp->m_pSavedSamples;
  else
    return false;
}
