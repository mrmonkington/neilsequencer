...
void C2PFilter::prewarp(float *r1, float *r2, 
			const float &a1, const float &a2) {
  *r2 = a2 / (m_fWP * m_fWP); // could turn one of these divides into a multiply
  *r1 = a1 / m_fWP;
}

void C2PFilter::bilinear(const float &a0, const float &a1, const float &a2,
			 const float &b0, const float &b1, const float &b2,
			 float *k, float *coef) {
  float ad, bd;
  ad = (4.f * a2 * m_fSampleRateSquared + 2.f * a1 * m_fSampleRate + a0);
  bd = (4.f * b2 * m_fSampleRateSquared + 2.f * b1 * m_fSampleRate + b0);
  *k *= ad / bd;
  *coef++ = ((2.f * b0 - 8.f * b2 * m_fSampleRateSquared) / bd);
  *coef++ = 
    ((4.f * b2 * m_fSampleRateSquared - 2.f * b1 * m_fSampleRate + b0) / bd);
  *coef++ = ((2.f * a0 - 8.f * a2 * m_fSampleRateSquared) / ad);
  *coef = 
    ((4.f * a2 * m_fSampleRateSquared - 2.f * a1 * m_fSampleRate + a0) / ad);
}

void C2PFilter::szxform(const C2PFilter::BIQUAD &Coef, 
			const float &i_b1, float *k, float *coef) {
  float	a1, a2;
  float	b1, b2;
...
void C2PFilter::Filter_Stereo(float **pOut, float **pIn, int iCount) {
  if (m_oBypass) {
    if (pOut != pIn) {
      memcpy(pOut[0], pIn[0], sizeof(float) * iCount);
      memcpy(pOut[1], pIn[1], sizeof(float) * iCount);
    }
    return;
  }
  while (iCount--) {
    *pOut[0]++ = iir_filter(*pIn[0]++, iirs[0]);
    *pOut[1]++ = iir_filter(*pIn[1]++, iirs[1]);
  }
}
