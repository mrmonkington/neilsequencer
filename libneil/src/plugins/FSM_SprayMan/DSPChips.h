...
/*       m_fAccum=value; */
/*     else */
/*       m_fAccum+=(float)_copysign(diff,value-m_fAccum); */
/*     return m_fAccum; */
/*   } */
/* }; */
...
public:
    ...
    inline SIG ProcessSample(SIG dSmp) {
        SIG dOut = m_b0 * dSmp + m_b1 * m_x1 + 
	  m_b2 * m_x2 - m_a1 * m_y1 - m_a2 * m_y2;
...
public:
    ...
    void SetFrequency(float dFreq, float dSampleRate) {
        ...
        m_fsd2fi=sin(m_fFreq);
        m_fcd2fi=cos(m_fFreq);
        m_fsd2nfi=sin(m_fFreq*m_fHarms);
        m_fcd2nfi=cos(m_fFreq*m_fHarms);
    }
    void GenerateSamples(SIG *pData, int nLen) {
        ...
        sinFi=sin(fi);
        cosFi=cos(fi);
        sinNFi=sin(m_fHarms*fi);
        cosNFi=cos(m_fHarms*fi);
        ...
        for (int i=0; i<nLen; i++) {
            ...
            pData[i]=SIG(val);
            EVOLVE_SINcos(sinFi2,cosFi2,sinFi,cosFi,m_fsd2fi,m_fcd2fi);
            sinFi=sinFi2;
            cosFi=cosFi2;
            EVOLVE_SINcos(sinNFi2,cosNFi2,sinNFi,cosNFi,m_fsd2nfi,m_fcd2nfi);
            sinNFi=sinNFi2;
            cosNFi=cosNFi2;
        }
    }
...
inline float CalcLFO(int nShape, float LFOPhase)
{
    ...
    case 0:
        ...
        break;
    case 1:
        LFO=(float)(((Phs-PI)/PI-0.5f)*2.0f);
        break;
    case 2:
        LFO=(float)(((Phs-PI)/PI-0.5f)*-2.0f);
        break;
    case 3:
        LFO=(Phs<PI)?1.0f:-1.0f;
        break;
    case 4:
        LFO=(float)(((Phs<PI?(Phs/PI):(2.0-Phs/PI))-0.5)*2);
        break;
    case 5:
        LFO=(float)sin(Phs*8*sin(Phs));
        break;
    case 6:
        LFO=(float)sin(Phs*2*pow(2,cos(Phs)));
        break;
    case 7:
        LFO=(float)sin(Phs*pow(4,sin(Phs)));
        break;
    case 8:
        LFO=(float)(0.5*sin(2*Phs)+0.5*cos(3*Phs));
        break;
    case 9:
        LFO=(float)(0.25*f2i(((Phs-PI)/PI-0.5f)*2.0f*4.0));
        break;
    case 10:
        LFO=(float)(-0.25*f2i(((Phs-PI)/PI-0.5f)*2.0f*4.0));
        break;
    case 11:
        LFO=(float)(0.125*f2i(((Phs-PI)/PI-0.5f)*2.0f*4.0)+0.5*fmod(Phs,PI/4)/(PI/4));
        break;
    case 12:
        LFO=(float)(-0.125*f2i(((Phs-PI)/PI-0.5f)*2.0f*4.0)+0.5*fmod(Phs,PI/4)/(PI/4));
        break;
    case 13:
        LFO=(float)(0.125*f2i(((Phs-PI)/PI-0.5f)*2.0f*4.0)+0.5*fmod(2*PI-Phs,PI/4)/(PI/4));
        break;
    case 14:
        LFO=(float)(-0.125*f2i(((Phs-PI)/PI-0.5f)*2.0f*4.0)+0.5*fmod(2*PI-Phs,PI/4)/(PI/4));
        break;
    case 15:
        LFO=(float)(0.5*(sin(19123*f2i(LFOPhase*8/PI)+40*sin(12*f2i(LFOPhase*8/PI)))));
        break; // 8 zmian/takt
    case 16:
        LFO=(float)(0.5*(sin(1239543*f2i(LFOPhase*4/PI)+40*sin(15*f2i(LFOPhase*16/PI)))));
        break; // 8 zmian/takt
    }
    return LFO;
}
...
{
    if (value==zzub::note_value_none || value==zzub::note_value_off) return value;
