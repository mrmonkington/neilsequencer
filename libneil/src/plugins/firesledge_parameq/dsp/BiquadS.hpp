...
#if defined (dsp_BiquadS_CURRENT_CODEHEADER)
	#error Recursive inclusion of BiquadS code header.
#endif
#define	dsp_BiquadS_CURRENT_CODEHEADER
...
float	BiquadS::get_sample_freq () const
{
	return (_sample_freq);
}
...
void	BiquadS::get_s_eq (float b [3], float a [3]) const
{
	assert (a != 0);
	assert (b != 0);
...
#undef dsp_BiquadS_CURRENT_CODEHEADER
