...
void Spectrum::drawGrid(cairo_t* cr, int n, int w, int h)
{
	...
	for(int i=0; i<(w - X_PAD); i+=(w - X_PAD)/WS) {
		...
		cairo_set_source_rgb(cr, 1, 1, 1);
		cairo_move_to(cr, X_PAD - 5 + i, h + 10);
		cairo_save(cr);
		cairo_rotate(cr, M_PI/2);
		char txt[10];
		// double f = i/float(w - X_PAD) * (n/2-1) * frange;
		// int bin = i / float(w - X_PAD) * (n/2);
		// double f = barkscale(bin, n, _master_info->samples_per_second);
		// f = f/2 / 13.500;

		// double f = i/float(w - X_PAD) * 13.5;
		// bin = 24 / (10 - bin);
		// double f = bark_bands[bin];
		// double f = barkscale(bin, n/2+1, _master_info->samples_per_second);
		
		// int bin = i / float(w - X_PAD) * 25;
		// sprintf(txt, "%d", bark_bands[bin]);
...
gboolean Spectrum::on_size_slider_changed(GtkWidget *widget, gpointer user_data) {
	((Spectrum*)user_data)->fftsize = (int)gtk_range_get_value(GTK_RANGE(widget));
	return TRUE;
}
