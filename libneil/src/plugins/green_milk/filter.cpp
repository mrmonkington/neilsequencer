...
char * OversampledDistortionFilter::describeFilterMode(enum FilterMode fm)
{
	switch(fm)
	{
...
char * OversampledDistortionFilter::describeOutput(int which)
{
	FilterMode m1,m2;
	FilterSequence seq;
	...
	sprintf(desc, "%s%s%s", type, describeFilterMode(m1), describeFilterMode(m2));
	return desc;
}
