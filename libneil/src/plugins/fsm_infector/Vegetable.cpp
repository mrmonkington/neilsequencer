...
namespace fsm {
  ...
  void CBandlimitedTable::Make(float fMultiplyFactor, float fMaxScanRate, float fCrispFactor) {
    ...
    int nSize = 1 << (int)(log((float)m_nBufSize) / log((float)2.0) + 0.999);
    assert(nSize == m_nBufSize); // XXXKF
    m_levels[0].m_nSize = nSize;
    m_levels[0].m_nBits = int(log((float)nSize) / log((float)2.0) + 0.5);
	...
	break;
      ...
      lev.m_nSize = nCount;
      lev.m_nBits = int(log((float)nCount) / log((float)2) + 0.5);
      ...
      for (int i = 0; i < nSize; i++)
	if (i >= std::min(nFirstSample,nLim))
	  pTemp2[i] = 0.0,
	    pTemp2[i + nSize] = 0.0;
	else
	  pTemp2[i] = pTemp[i],
