...
{
	if (RetrigCount)
		maxtime=std::min(RetrigCount-RetrigPoint,maxtime);
...
static float SustLevel(unsigned char value)
{
  return (float)pow(1.0/128,1-value/240.0);
}

unsigned char CTrack::AllocChannel()
{
	int i;
...
void CTrack::PlayNote(unsigned char note, unsigned char _accent, unsigned char _length, zzub::master_info *pMasterInfo)
{
...
/*
  ...
  double length=(_length==240)?32768:double(_length)*pMasterInfo->samples_per_tick/(16*pMasterInfo->samples_per_second);
	RetrigPoint=0;

  gvals &gvalAct=pmi->gvalAct;
  chn->FilterEnv.SetEnvelope(GETENVTIME2(gvalAct.vFilterAttack),GETENVTIME(gvalAct.vFilterDecay),SustLevel(gvalAct.vFilterSustain),length,GETENVTIME(gvalAct.vFilterRelease),pMasterInfo->samples_per_second);
  chn->AmpEnv.SetEnvelope(GETENVTIME2(gvalAct.vAmpAttack),GETENVTIME(gvalAct.vAmpDecay),SustLevel(gvalAct.vAmpSustain),length,GETENVTIME(gvalAct.vAmpRelease),pMasterInfo->samples_per_second);
...
//		else
	...
	SlideEnd=0.0;
  chn->Detune=Detune;
  BaseFrequency=DestFrequency=float((220*pow(2.0,((note-1)>>4)+((note&15)-22-36)/12.0))/pMasterInfo->samples_per_second);
	if (!gvalAct.vGlide)
...
void CTrack::DoLFO(fsm_infector *pmi, int c)
{
  CurLFO=(float)(inrLFO1.Process(float(CalcLFO(pmi->gvalAct.vLFOShape,LFOPhase)*30.0),c)/30.0);

  float DeltaPhase;
  if (pmi->gvalAct.vLFORate<240)
    DeltaPhase = (float)(2*3.1415926*LFOPAR2TIME(pmi->gvalAct.vLFORate)/pmi->_master_info->samples_per_second);
  else
    DeltaPhase = (float)(2*3.1415926*(float(pmi->_master_info->ticks_per_second))/(times[pmi->gvalAct.vLFORate-240]*pmi->_master_info->samples_per_second));
  ...
  if (pmi->gvalAct.vLFO2Rate<240)
    DeltaPhase = (float)(2*3.1415926*LFOPAR2TIME(pmi->gvalAct.vLFO2Rate)/pmi->_master_info->samples_per_second);
  else
    DeltaPhase = (float)(2*3.1415926*(float(pmi->_master_info->ticks_per_second))/(times[pmi->gvalAct.vLFO2Rate-240]*pmi->_master_info->samples_per_second));
...
void CTrack::DoWakeup(fsm_infector *pmi)
{
	if (RetrigCount && RetrigPoint>=RetrigCount)
	{
		RetrigPoint=0;
		if (Arps[ArpPoint%ArpCount]!=char(0x80))
			PlayNote(SeqToBuzz(BuzzToSeq(lastnote)+Arps[ArpPoint%ArpCount]),lastaccent,lastlength,pmi->_master_info);
		lastnote-=Arps[ArpPoint%ArpCount];
		ArpPoint=(ArpPoint+1)%ArpCount;
	}
}

void CTrack::CommandA(unsigned char cmd, unsigned short param)
{
	if (cmd==0xFE)
    pmi->ClearFX();
	if (cmd==0xFD)
	...
	if (cmd==0xED)
	{
		RetrigCount=pmi->_master_info->samples_per_tick;
		RetrigPoint=RetrigCount-(param&0x0F)*pmi->_master_info->samples_per_tick/12;
		NoTrig=1;
		RetrigMode=0;
		Arps[0]=0;
		ArpPoint=0; ArpCount=1;
...
void CTrack::CommandB(unsigned char cmd, unsigned short param)
{
	if (cmd==0x01)
	{
		if (SlideEnd) BaseFrequency=Chn()->Frequency;
		SlideCounter=0;
		SlideRange=(unsigned char)(param);
    ...
    if (Chn())
    {
      if (SlideEnd) BaseFrequency=Chn()->Frequency;
		  SlideCounter=0;
		  SlideRange=float(12*log(NotePortFrequency/Chn()->Frequency)/log((float)2));
	...
	if (cmd==0x0C)
	{
		if ((unsigned char)(param)!=0xFF) LFOPhase=float(param*2*PI/255);
		if ((unsigned char)(param>>8)!=0xFF) LFO2Phase=float(param*2*PI/255);
	}
	if (cmd==0xE5)
	{
	...
	if (cmd==0x05)
	{
		RetrigCount=pmi->_master_info->samples_per_tick/3+1;
		RetrigPoint=0;
		RetrigMode=0;
	...
	if (cmd==0x06)
	{
		RetrigCount=pmi->_master_info->samples_per_tick+1;
		RetrigPoint=(12-(param>>8))*pmi->_master_info->samples_per_tick/12;
		RetrigMode=0;
	...
	if (cmd==0xE9)
	{
		RetrigCount=int((param&0x0F)*(pmi->_master_info->samples_per_tick+1)/12.0+0.99);
		RetrigPoint=((param&0xF0)>>4)*pmi->_master_info->samples_per_tick/12;
		RetrigMode=0;
		Arps[0]=0;
		ArpPoint=0; ArpCount=1;
	}
